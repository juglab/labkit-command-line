 ...
 */
@CommandLine.Command(name = "show", description = "Show the segmentation stored in the N5 folder using Big Data Viewer.")
public class ShowCommand implements Callable<Optional<Integer>> {

	@CommandLine.Option(names = {
			"--n5"}, description = "N5 folder that was used to store the segmentation.", required = true)
	private File n5;

	@Override
	public Optional<Integer> call() throws Exception {
		N5FSReader reader = new N5FSReader(n5.getAbsolutePath());
		RandomAccessibleInterval<UnsignedByteType> result = N5Utils.open(reader, PrepareCommand.N5_DATASET_NAME,
