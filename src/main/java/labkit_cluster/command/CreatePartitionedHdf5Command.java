 ...
 */
@CommandLine.Command(name = "create-partitioned-hdf5", description = "Save the segmentation stored in the N5 folder as multiple Big Data Viewer compatible HDF5.")
public class CreatePartitionedHdf5Command implements Callable<Optional<Integer>> {
	...
	@CommandLine.Option(names = {"-H", "--header"}, description = "Only write XML & index HDF5" )
	...
	public Optional<Integer> call() throws Exception {
		...
		else if ( partitionIndex != null )
			saver.writePartition(partitionIndex);
		else
			saver.writeAll();
